...
bln = dic[(str(datetime.datetime.n #FF00BCow().month))]
...
def login():
		try:
			...
			try:
				...
				banner()
				li = '# ada masalah sambungan internet silahkan coba lagi
...
def login_lagi334():
	try:
		...
		print('%s# LOGIN GAGAL ! COOKIE ANDA BASI'%(h))
		exit()
...
def menu(my_name,my_id):
	...
	io = '''[bold cyan][01] CRACK DARI AKUN PUBLIK	     [06] CRACK DARI FILES
[02] CRACK MASAL  [07] CHECK CHECKPOINT OPTIONS
[03] FOLLOWER		     [08] CHECK CRACK RESULTS
[04] LIKES THE POST	     [09] TIPS CRACK
[05] CRACK DARI ANGGOTA GRUB           [00] KELUAR DARI SCRIPT[bold cyan]'''
...
def crackmbasic(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%sOK NIH PASTI %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
...
if __name__=='__main__':
	...
	os.system("exit")
	os.system(python)
	fikri_xy(f//'√tools untuk crack [gunakan sewajarnya ajah]